...
from IOS_Calc_buttons import Button, ImageButton, NumButton, MathButton
import darkdetect
from PIL import Image
from IOS_Calc_Settings import *
try:
...
class Calculator(ctk.CTk):
    def __init__(self, is_dark):
        ...
        #setup
        super().__init__()
        # 1. set appearence to dark or light depending on is_dark
        # ctk.set_appearance_mode(f'{"dark" if is_dark else "light"}') # LIGHT MODE
        ctk.set_appearance_mode(f'{"light" if is_dark else "dark"}') # DARK MODE
        # 2. fg_color = WHITE or BLACK
        super().__init__(fg_color = (WHITE, BLACK))
        # 3. get those start window size from the settings and disable window resizing
        self.geometry(f'{APP_SIZE[0]}x{APP_SIZE[1]}')
        self.resizable(False, False)
        # 4. hide the title and the icon
        self.title("")
        # self.iconbitmap("empty.ico")
        self.iconbitmap("logo_gpt.ico")
    ...
    # def num_press(self, value):
    #     """
    #     Handles number button presses, updating the current number being entered.
    #
    #     Args:
    #         value (str): The number value pressed.
    #     """
    #     self.display_nums.append(str(value))
    #     full_number = "".join(self.display_nums)
    #     formatted_number = "{:,}".format(int(full_number)).replace(",", " ")
    #     self.result_string.set(formatted_number)
